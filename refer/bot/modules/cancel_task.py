...
from .. import task_dict, task_dict_lock, user_data, multi_tags, LOGGER
...
from ..helper.telegram_helper.sticker_utils import send_error_sticker
...
async def cancel(_, message):
    ...
    # Send a sad sticker when a task is cancelled by command
    try:
        await send_error_sticker(message)
    except Exception:
        pass
...
async def cancel_multi(_, query):
    ...
    if tag in multi_tags:
        ...
        # Clean up video tools selection for this multi-download
        from ..helper.common import multi_video_tools_selection, pending_multi_downloads

        # The multi_tag used in video tools selection is the hex string directly
        # Since multi_tags contains integer values, we need to check if there's a corresponding hex tag
        hex_tag = f"{tag:06x}"
        if hex_tag in multi_video_tools_selection:
            del multi_video_tools_selection[hex_tag]
            LOGGER.info(
                f"Cleaned up video tools selection for cancelled multi-download: {hex_tag}"
            )
        # Clean up pending status
        if hex_tag in pending_multi_downloads:
            del pending_multi_downloads[hex_tag]
            LOGGER.info(
                f"Cleaned up pending status for cancelled multi-download: {hex_tag}"
            )
        msg = "Stopped!"
    else:
...
def create_cancel_buttons(is_sudo, user_id=""):
    ...
    buttons.data_button(
        "QueuedMedia", f"canall ms {MirrorStatus.STATUS_QUEUEMEDIA} {user_id}"
    )
    buttons.data_button(
    ...
    buttons.data_button(
        "Watermark", f"canall ms {MirrorStatus.STATUS_WATERMARK} {user_id}"
    )
