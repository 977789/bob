    ...
    async def new_event(self):
        ...
        if not getattr(self, "returning_from_thumbnail", False):
            ...
            if "-t" in input_list and self.thumb == "":
                ...
                # Create a Mirror object to handle the thumbnail request
                mirror_obj = Mirror(self.client, self.message)
                mirror_obj.thumb = ""
                # Set this to our object so when the thumbnail is uploaded it continues correctly
                mirror_obj.yt_obj = self
                await mirror_obj.request_thumbnail()
                return
        ...
        user_cookies_path = f"cookies/{self.user_id}.txt"
        import os

        cookies_file = (
            user_cookies_path if os.path.exists(user_cookies_path) else "cookies.txt"
        )
