...
async def get_buttons(key=None, edit_type=None, edit_mode=False, user_id=None):
    ...
    if key is None:
        ...
        for k in list(conf_dict.keys())[start : 10 + start]:
            if k in SENSITIVE_KEYS and not is_owner(user_id):
                continue
...
async def update_buttons(
    ...
    await update_message_with_photo(
        message, msg, button, photo="assets/BHARTIYEE LEECH.png"
    )
...
async def edit_variable(_, message, pre_message, key):
    ...
    if value.lower() == "true":
        ...
        value = int(value)
    elif key == "DL_SPEED_LIMIT":
        # Accept simple numbers (1, 2, 3, 4, 5) representing MB/s
        try:
            if isinstance(value, str) and value.strip():
                ...
                if value.strip().isdigit():
                    ...
                    # Support legacy format like "500kb", "2mb" but convert to MB/s
                    from ..helper.ext_utils.bot_utils import get_size_bytes

                    bytes_val = get_size_bytes(value)
                    parsed = max(0, bytes_val // 1048576)  # Convert bytes to MB
            else:
                parsed = int(value) if value else 0

            # Ensure it's a reasonable value (0-100 MB/s)
            value = max(0, min(parsed, 100))
        except Exception:
            await send_message(
                message,
                "Invalid value! Use a simple number for MB/s (e.g., 1 for 1MB/s, 5 for 5MB/s, max 100).",
            )
            return await update_buttons(pre_message, "var")
    elif key == "SPEED_LIMIT_TIMEOUT":
        try:
            value = int(value)
            if value < 60 and value > 0:  # Minimum 1 minute if not disabled
                value = 60
        except Exception:
            await send_message(
                message,
                "Invalid value! Use a number in seconds (minimum 60).",
            )
            return await update_buttons(pre_message, "var")
    elif key == "LEECH_SPLIT_SIZE":
        # Support values like 500mb, 2gb, or raw bytes
        try:
            if isinstance(value, str) and not value.isdigit():
                from ..helper.ext_utils.bot_utils import get_size_bytes

                parsed = get_size_bytes(value)
            else:
                parsed = int(value)
        except Exception:
            await send_message(
                message,
                "Invalid value! Use a number (bytes) or size with units like 500mb or 2gb.",
            )
            return await update_buttons(pre_message, "var")
        # Clamp to client-supported maximum
        value = min(max(0, parsed), TgClient.MAX_SPLIT_SIZE)
        ...
        # CMD_SUFFIX should always remain a string, even if it's numeric
        value = str(value)
    elif value.isdigit():
        value = int(value)
...
async def edit_bot_settings(client, query):
    ...
    if data[1] == "close":
        ...
        elif data[2] == "DL_SPEED_LIMIT":
            value = 0
        elif data[2] == "SPEED_LIMIT_TIMEOUT":
...
async def send_bot_settings(_, message):
    ...
    globals()["start"] = 0
    await send_message(message, msg, button, photo="assets/BHARTIYEE LEECH.png")
