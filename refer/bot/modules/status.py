...
async def status_pages(_, query):
    ...
    # Check if this is an old status message (from before bot restart)
    async with task_dict_lock:
        if key not in status_dict:
            # This is an old status message - show popup notification
            await query.answer(
                "⚠️ Status Expired!\n\n"
                "This status message is expired and no longer active.\n"
                "Please generate a new status using /s or /status command.",
                show_alert=True,
            )
            return
    ...
    if data[2] == "ref":
        ...
        # If less than 6 seconds have passed since last refresh
        if current_time - last_refresh < 6:
            remaining_time = 6 - int(current_time - last_refresh)
            await query.answer(
                f"Please wait {remaining_time} seconds before refreshing again.",
                show_alert=True,
            )
            return

        # Update last refresh time for this user
        refresh_cooldown[user_id] = current_time
        ...
        for status, speed, _ in status_results:
            match status:
                ...
                case MirrorStatus.STATUS_QUEUEUP:
                    tasks["QueueUp"] += 1
                case MirrorStatus.STATUS_QUEUEMEDIA:
                    tasks["QueueMedia"] += 1
