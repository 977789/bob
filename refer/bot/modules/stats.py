...
async def get_stats(event, key="home"):
    ...
    if key == "home":
        ...
        # Only show Repo Stats button to owner
...
"""
        ...
        # Only allow owner to access repo stats
        if user_id != Config.OWNER_ID:
            msg = "〄 <b><i>Access Denied!</i></b>\n\n⚠️ Only the bot owner can view repository statistics."
        else:
...
┊<b>Download Speed Limit :</b> {f"{Config.DL_SPEED_LIMIT} MB/s" if Config.DL_SPEED_LIMIT > 0 else "∞"}
┊<b>Speed Limit Timeout :</b> {get_readable_time(Config.SPEED_LIMIT_TIMEOUT) if Config.DL_SPEED_LIMIT > 0 else "N/A"}
...
async def stats_pages(_, query):
    ...
    elif data[2] == "strepo" and user_id != Config.OWNER_ID:
        await query.answer(
            "Access Denied! Only owner can view repo stats.", show_alert=True
        )
    else:
        await query.answer()
