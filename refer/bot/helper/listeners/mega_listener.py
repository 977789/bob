    ...
    def onRequestFinish(self, api, request, error):
        ...
        if request_type == MegaRequest.TYPE_LOGIN:
            ...
            if self.node:
                ...
                LOGGER.error("Failed to get root node from FETCH_NODES")
        else:
            # For other request types (like folder login), try to get root node
            if self.node is None:
                root_node = api.getRootNode()
                if root_node:
                    self.node = root_node
                    self._name = self.node.getName()
                    LOGGER.info(f"Set node from root: {self.node.getName()}")
