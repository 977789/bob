...
from .speed_monitor import init_speed_monitoring, cleanup_speed_monitoring
...
async def _on_download_started(api, data):
    ...
    if task := await get_task_by_gid(gid):
        ...
        # Initialize speed monitoring for this download
        init_speed_monitoring(gid, task.listener.name)


async def _on_download_complete(api, data):
    try:
        gid = data["params"][0]["gid"]
        cleanup_speed_monitoring(gid)  # Clean up speed monitoring
        download, options = await api.tellStatus(gid), await api.getOption(gid)
        if options.get("follow-torrent", "") == "false":
            return
