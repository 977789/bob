...
import re
import time
from re import findall, match, search, compile
...
async def get_links(session, url):
    ...
    if loc and "link=" in loc:
        return loc.split("link=")[-1]
    return loc


async def bypass_hubcloud(session, url):
    print(f"[Hubcloud] Processing: {url}")
    res = await session.get(url)
    text = await res.text()
    link = search(r"url = '(.*?)'", text)
    if not link:
        print("Hubcloud intermediate URL not found.")
        return
    res = await session.get(link.group(1))
    soup = BeautifulSoup(await res.text(), "html.parser")
    for i in soup.find_all("a", {"class": compile(r"btn.*")}):
        if "?id=" in i["href"]:
            final = await get_links(session, i["href"])
            print(f"[Hubcloud] Final Link: {final}")
            return final
        else:
            print(f"[Hubcloud] Direct Link: {i.get('href')}")
            return i.get("href")


async def bypass_hubdrive(session, url):
    res = await session.get(url)
    html = await res.text()
    soup = BeautifulSoup(html, "html.parser")

    button = soup.find("a", class_="btn btn-primary btn-user btn-success1 m-1")
    if not button:
        return

    hubcloud_url = button["href"]
    return await bypass_hubcloud(session, hubcloud_url)


async def bypass_auto(url):
    async with ClientSession() as session:
        if "hubcloud" in url:
            return await bypass_hubcloud(session, url)
        elif "hubdrive" in url:
            return await bypass_hubdrive(session, url)
        else:
...
def hubcloud(url):
    ...
    try:
        ...
        return loop.run_until_complete(bypass_auto(url))
...
# GDLink/GDFlix bypass functionality
SOURCE_DOMAIN = "gdlink.dev"
TARGET_DOMAIN = "new4.gdflix.dad"


def convert_gdlink_url(url):
    """Convert source domain to target domain in a URL."""
    return url.replace(SOURCE_DOMAIN, TARGET_DOMAIN)


async def fetch_gdlink_page(session, url):
    ...
    try:
        async with session.get(url, headers=headers) as response:
            if response.status != 200:
                print(f"[GDLink] Failed to fetch page: HTTP {response.status}")
                return None
            html = await response.text()
            print(f"[GDLink] Successfully fetched page: {len(html)} bytes")
            return html
    except Exception as e:
        print(f"[GDLink] Error fetching page: {e}")
        return None


async def get_gdlink_download_links(url):
    """Extract download links from the GDFlix page."""
    url = convert_gdlink_url(url)
    async with ClientSession() as session:
        html = await fetch_gdlink_page(session, url)
        if not html:
            print("[GDLink] No HTML content to parse.")
            return None

        soup = BeautifulSoup(html, "html.parser")
        print("[GDLink] HTML parsed successfully.")

        download_section = soup.find("div", class_="text-center")
        if not download_section:
            print("[GDLink] No 'text-center' div found, searching entire page.")
            download_section = soup

        buttons = download_section.find_all("a", class_=lambda x: x and "btn" in x)
        if not buttons:
            print("[GDLink] No buttons with 'btn' class found.")
            return None

        print(f"[GDLink] Found {len(buttons)} buttons with 'btn' class.")
        ...
        for button in buttons:
            ...
            full_url = (
                convert_gdlink_url(href)
                if href.startswith("http")
                else f"https://{TARGET_DOMAIN}{href}"
            )
            button_text = button.get_text(strip=True)
            print(f"[GDLink] {button_text}: {full_url}")

            # Return the first valid download link found
            if href and ("download" in button_text.lower() or "drive" in href):
                return full_url
...
def gdlink(url):
    ...
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(get_gdlink_download_links(url))
        if result:
            return result
        else:
...
def oxxfile(url):
    ...
    try:
        ...
        api_url = f"https://new.oxxfile.info/api/s/{file_id}/"
        ...
        # Check if the response contains download information
        if "download_url" in data:
            print(f"[OxxFile] Found download URL: {data['download_url']}")
            return data["download_url"]
        elif "url" in data:
            print(f"[OxxFile] Found URL: {data['url']}")
            return data["url"]
        elif "file_url" in data:
            print(f"[OxxFile] Found file URL: {data['file_url']}")
            return data["file_url"]
        else:
            # Print the data structure for debugging
            print(f"[OxxFile] API Response: {data}")

            # Try to find any URL-like field in the response
            for key, value in data.items():
                if isinstance(value, str) and (
                    value.startswith("http") or "download" in key.lower()
                ):
                    print(
                        f"[OxxFile] Found potential download link in '{key}': {value}"
                    )
                    return value

            raise DirectDownloadLinkException("No download URL found in API response")
...
def direct_link_generator(link):
    ...
    elif "gdlink.dev" in domain or "gdflix" in domain:
        return gdlink(link)
    elif "oxxfile.info" in domain:
        return oxxfile(link)
...
def gofile(url):
    ...
    if len(details["contents"]) == 1:
        ...
        LOGGER.info(
            f"GoFile single file: URL={details['contents'][0]['url']}, Header={details['header']}"
        )
        return (details["contents"][0]["url"], details["header"])
    from .... import LOGGER

    LOGGER.info(
        f"GoFile multiple files: {len(details['contents'])} files, Header={details['header']}"
    )
    return details
