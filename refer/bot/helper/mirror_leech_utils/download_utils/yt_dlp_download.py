...
from ....helper.listeners.speed_monitor import (
    init_speed_monitoring,
    cleanup_speed_monitoring,
    check_speed_limit,
)
...
class YoutubeDLHelper:
    def __init__(self, listener):
        ...
        user_cookies_path = (
            f"cookies/{getattr(listener, 'user_id', None)}.txt"
    ...
    def _on_download_progress(self, d):
        ...
        if d["status"] == "finished":
            ...
            # Check speed limit for YT-DLP downloads
            if hasattr(self, "_gid"):
                import asyncio

                try:
                    # Run speed check in a non-blocking way
                    loop = asyncio.get_event_loop()
                    if loop.is_running():
                        # If we're already in an async context, schedule the task
                        asyncio.create_task(self._check_ytdlp_speed())
                except Exception:
                    pass  # Ignore async context errors
    ...
    async def _on_download_start(self, from_queue=False):
        ...
        # Initialize speed monitoring for YT-DLP
        init_speed_monitoring(self._gid, self._listener.name)
    ...
    async def _check_ytdlp_speed(self):
        """Check YT-DLP download speed against limit"""
        if await check_speed_limit(self._gid, int(self._download_speed)):
            # Speed is too slow, cancel the download
    ...
    def _on_download_error(self, error):
        cleanup_speed_monitoring(self._gid)  # Clean up speed monitoring
        self._listener.is_cancelled = True
