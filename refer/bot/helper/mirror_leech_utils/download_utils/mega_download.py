...
async def add_mega_download(listener, path):
    async_api = AsyncMega()
    async_api.api = api = MegaApi(None, None, None, "Bzex")
    folder_api = None
    ...
    if get_mega_link_type(listener.link) == "file":
        ...
        async_api.folder_api = folder_api = MegaApi(None, None, None, "Bzex")
        folder_api.addListener(mega_listener)

        await async_api.run(folder_api.loginToFolder, listener.link)

        # Check if node was set properly after login
        if mega_listener.node is None:
            # Try to get the root node directly as a fallback
            try:
                mega_listener.node = folder_api.getRootNode()
                if mega_listener.node:
                    LOGGER.info(
                        f"Retrieved root node as fallback: {mega_listener.node.getName()}"
                    )
                else:
                    await listener.on_download_error(
                        "Failed to get folder node after login"
                    )
                    await async_api.logout()
                    return
            except Exception as e:
                await listener.on_download_error(f"Failed to get folder node: {str(e)}")
                await async_api.logout()
                return
