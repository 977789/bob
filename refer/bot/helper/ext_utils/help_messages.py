...
filename_remover = """<b>Remove/Replace Filename Patterns</b>: -remname

<b>Pattern Removal (Old Style):</b>
/cmd link -remname [WEB-DL]|RARBG|1080p (remove multiple patterns separated by |)

<b>Pattern Replacement (New Style):</b>
/cmd link -remname "(Yao.Shen.Ji)|rizz (Yao.Shen)|iss" (replace patterns with text)
/cmd link -remname "pattern1|replacement1 pattern2|replacement2"

<b>Mixed Operations:</b>
/cmd link -remname "(Yao.Shen.Ji)|rizz unwanted_text" (replace first, remove second)

<b>Syntax Rules:</b>
• <code>pattern</code> - Remove pattern completely
• <code>pattern|replacement</code> - Replace pattern with replacement text
• <code>pattern|</code> - Replace pattern with empty (same as removal)
• Use spaces to separate different pattern rules
• Works with files and folders
• Case-insensitive matching
• Can use user's default patterns from /usetting

<b>Examples:</b>
Remove patterns: -remname "[WEB-DL] RARBG 1080p"
Replace patterns: -remname "(Yao.Shen.Ji)|Rizz (Chinese)|ENG"
Mixed: -remname "(Yao.Shen.Ji)|Rizz RARBG Sample"

<b>Your Current Pattern:</b>
<code>(Yao.Shen.Ji)|rizz</code> means: Replace "(Yao.Shen.Ji)" with "rizz"

<b>Note:</b> Pattern processing is applied before other filename operations.
"""
...
<b>Enable SS Grid feature</b>: -ssg
/cmd link -ssg

<b>Set SS Grid screenshot count</b>: -ssgc
/cmd link -ssg -ssgc 12 (takes 12 screenshots)

<b>Set SS Grid layout</b>: -ssgl
/cmd link -ssg -ssgl 3x4 (creates a grid with 3 rows and 4 columns)

<b>Enable PDF mode for SS Grid</b>: -ssgp
/cmd link -ssg -ssgp (combines screenshots into a PDF)

<b>Add PDF watermark for SS Grid</b>: -ssgw
/cmd link -ssg -ssgp -ssgw "My Watermark" (adds watermark text to the PDF)

<b>Full example</b>:
/cmd link -ssg -ssgc 12 -ssgl 3x4 -ssgp -ssgw "Downloaded by Bot"
"""
...
Requires user's personal token (via /usetting) or global GOFILE_API token as fallback.

<b>Personal GoFile Settings:</b>
• Set personal token: /usetting → Mirror Settings → GoFile Tools → GoFile Token
• Set upload folder: /usetting → Mirror Settings → GoFile Tools → GoFile Folder ID
• Switch upload mode: /usetting → General Settings → Swap to GoFile Mode

<b>Token Priority:</b> User's personal token → Global bot token
<b>Folder Priority:</b> User's folder ID → Root folder"""
...
force_video_tools = """<b>Force Video Tools</b>: -ft

/cmd link -ft (interactive video processing selection)

When you use the -ft flag, the bot will show an interactive menu with buttons allowing you to select which video processing operations you want to perform:

<b>Available Operations:</b>
• <b>Video Encode</b> - Re-encode videos with different settings (supports multi-resolution encoding)
• <b>Video Watermark</b> - Add text/image watermark to videos  
• <b>Video+Audio Merge</b> - Merge separate audio tracks with video
• <b>Video+Subtitle Merge</b> - Merge subtitle files with video (soft subtitles)
• <b>Video Hardsub</b> - Burn subtitle files permanently into video
• <b>Video Merge</b> - Merge multiple video files into one
• <b>Stream Extract</b> - Extract audio/subtitle streams from video
• <b>Stream Swap</b> - Reorder audio/subtitle tracks
• <b>Stream Remove</b> - Remove unwanted audio/subtitle tracks

<b>Hardsub Feature:</b>
Permanently burns subtitle files into video, creating a video with built-in subtitles:
• Supports SRT, ASS, SSA, VTT subtitle formats
• Customizable font size, style, and appearance
• Subtitles become part of the video (cannot be disabled)
• Ideal for compatibility with devices that don't support external subtitles
• Configure styling options in user settings (/usetting)

<b>Multi-Resolution Encoding:</b>
When enabled in user settings, Video Encode will create multiple quality versions:
• 1080p, 720p, 576p, 480p, 360p (based on original resolution)
• Each version optimized with appropriate bitrates
• Only creates resolutions equal to or lower than original

<b>Multi-Zip Feature:</b>
When both Multi-Resolution and Multi-Zip are enabled:
• All encoded resolutions are packaged into a single zip file
• Creates a convenient archive named "filename_MultiRes_Encodes.zip"
• Reduces clutter by combining all quality versions into one file
• Ideal for distributing multiple resolution options together

<b>Interactive Selection:</b>
- Toggle individual tools on/off with buttons
- Select all or deselect all options
- Confirm your selection to proceed
- Cancel to abort the download

Only tools enabled in your user settings (/usetting) will be available for selection. The download will proceed with only the operations you select."""
...
YT_HELP_DICT = {
    ...
    "Force-Video-Tools": force_video_tools,
    "Name-Swap": name_swap,
    "Filename-Remover": filename_remover,
...
/{BotCommands.RestartCommand}: Restart and update the bot. Use with 'taskcomplete' or 'tc' to wait for all tasks to finish first (Only Owner & Sudo).
...
BOT_COMMANDS = {
    ...
    "SetThumb": "[reply/link] Set or clear your custom thumbnail (/setthumb <link> or reply). Use /setthumb none to clear.",
    ...
    "Restart": "[SUDO] Reboot bot. Use 'taskcomplete' or 'tc' to wait for tasks",
