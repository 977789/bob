...
# Dict to store Mirror objects waiting for thumbnail uploads
thumbnail_waiters = {}

# Dict to store listeners waiting for trim time input {user_id: {listener, prompt_msg_id}}
trim_waiters = {}
            ...
            if condition and (limit := getattr(Config, attr, 0)):
                if attr == "PLAYLIST_LIMIT":
                    if yt_playlist >= limit:
                        limit_exceeded = f"\n┊{name} Limit Count » {limit}"
                else:
                    byte_limit = limit * 1024**3
                    if size >= byte_limit:
                        limit_exceeded = (
                            f"\n┊{name} Limit » {get_readable_file_size(byte_limit)}"
                        )
    ...
    if not limit_exceeded:
        ...
        if Config.STORAGE_LIMIT and not listener.is_clone:
            ...
            if not await check_storage_threshold(
                ...
                limit_exceeded = (
                    f"\n┊Threshold Storage Limit » {get_readable_file_size(limit)}"
                )
...
async def pre_task_check(message):
    ...
    # Always return a tuple to avoid TypeError
    return None, None


async def check_media_processing_tasks(listener):
    """Check if media processing task should be queued based on QUEUE_MEDIA_PROCESSING limit"""
    media_limit = Config.QUEUE_MEDIA_PROCESSING
    event = None
    is_over_limit = False

    async with queue_dict_lock:
        if media_limit and not getattr(listener, "force_media_processing", False):
            current_count = len(non_queued_media_processing)
            is_over_limit = current_count >= media_limit

            if is_over_limit:
                event = Event()
                queued_media_processing[listener.mid] = event
                LOGGER.info(
                    f"Media processing task queued: {listener.name} (Current: {current_count}/{media_limit})"
                )
            else:
                non_queued_media_processing.add(listener.mid)
                LOGGER.info(
                    f"Media processing task started: {listener.name} (Current: {current_count + 1}/{media_limit})"
                )
        else:
            # No limit or force processing
            non_queued_media_processing.add(listener.mid)

    return is_over_limit, event


async def start_media_processing_from_queued(mid: int):
    """Start a media processing task from queue"""
    if mid in queued_media_processing:
        queued_media_processing[mid].set()
        del queued_media_processing[mid]
        non_queued_media_processing.add(mid)
        LOGGER.info(f"Started media processing from queue: {mid}")


async def finish_media_processing_task(mid: int):
    """Mark media processing task as finished and start next from queue"""
    async with queue_dict_lock:
        if mid in non_queued_media_processing:
            non_queued_media_processing.remove(mid)
            LOGGER.info(f"Finished media processing task: {mid}")

    # Start next task from queue if any
    await start_media_processing_from_queued_list()


async def start_media_processing_from_queued_list():
    """Start media processing tasks from queue based on available slots"""
    media_limit = Config.QUEUE_MEDIA_PROCESSING
    if not media_limit:
        return
    ...
    async with queue_dict_lock:
        current_count = len(non_queued_media_processing)
        available_slots = media_limit - current_count

        if available_slots > 0 and queued_media_processing:
            for mid in list(queued_media_processing.keys())[:available_slots]:
                await start_media_processing_from_queued(mid)

    return None, None


# ---------------- Extraction Queue Management ---------------- #
async def check_extraction_tasks(listener):
    """Check if extraction task should be queued based on QUEUE_EXTRACT limit."""
    extract_limit = Config.QUEUE_EXTRACT
    event = None
    is_over_limit = False

    async with queue_dict_lock:
        if extract_limit and extract_limit > 0:
            current = len(non_queued_extract)
            is_over_limit = current >= extract_limit
            if is_over_limit:
                event = Event()
                queued_extract[listener.mid] = event
                LOGGER.info(
                    f"Extraction task queued: {listener.name} (Current: {current}/{extract_limit})"
                )
            else:
                non_queued_extract.add(listener.mid)
                LOGGER.info(
                    f"Extraction task started: {listener.name} (Current: {current + 1}/{extract_limit})"
                )
        else:
            non_queued_extract.add(listener.mid)
    return is_over_limit, event
...
async def start_extraction_from_queued(mid: int):
    if mid in queued_extract:
        queued_extract[mid].set()
        del queued_extract[mid]
        non_queued_extract.add(mid)
        LOGGER.info(f"Started extraction from queue: {mid}")


async def finish_extraction_task(mid: int):
    async with queue_dict_lock:
        if mid in non_queued_extract:
            non_queued_extract.remove(mid)
            LOGGER.info(f"Finished extraction task: {mid}")
    await start_extraction_from_queued_list()


async def start_extraction_from_queued_list():
    extract_limit = Config.QUEUE_EXTRACT
    if not extract_limit:
        return
    async with queue_dict_lock:
        current = len(non_queued_extract)
        available = extract_limit - current
        if available > 0 and queued_extract:
            for mid in list(queued_extract.keys())[:available]:
                await start_extraction_from_queued(mid)
