...
async def clean_download(opath):
    LOGGER.info(f"[CLEAN] clean_download called for: {opath}")

    # Check if there are any active extractions using this path
    try:
        from .stream_extractor import active_extractions

        for session_id, data in active_extractions.items():
            extract_path = data.get("path", "")
            if extract_path.startswith(opath) or opath in extract_path:
                LOGGER.info(
                    f"[CLEAN] Skipping cleanup for {opath} as it's being used by active extraction: {session_id}"
                )
                return
    except ImportError:
        pass
    except Exception as e:
        LOGGER.error(f"[CLEAN] Error checking active extractions: {str(e)}")

    if await aiopath.exists(opath):
        LOGGER.info(f"[CLEAN] {opath} exists, starting cleanup")
        try:
            LOGGER.info(f"[CLEAN] Before aiormtree for {opath}")
            await aiormtree(opath, ignore_errors=True)
            LOGGER.info(f"[CLEAN] After aiormtree for {opath}")
        except Exception as e:
            LOGGER.error(f"[CLEAN] Exception in clean_download for {opath}: {str(e)}")
    else:
        LOGGER.info(f"[CLEAN] {opath} does not exist, skipping cleanup")
...
class SevenZ:
    ...
    async def extract(self, f_path, t_path, pswd):
        ...
        threads = getattr(Config, "EXTRACT_THREADS", 1) or 1
        cmd = [
            "7z",
            "x",
            f"-p{pswd}",
            ...
            f"-o{t_path}",
            "-aot",
            f"-mmt={threads}",  # limit threads to control memory usage
            "-xr!@PaxHeader",
            "-bsp1",
            "-bse1",
            "-bb3",
