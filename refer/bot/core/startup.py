...
from ..helper.ext_utils.hyperdl_checker import (
    check_hyperdl_status,
    log_hyperdl_requirements,
)
...
async def update_variables():
    # Always use the maximum split size available
    if TgClient.IS_PREMIUM_USER:
        # For premium users, always set to 4GB
        Config.LEECH_SPLIT_SIZE = 4194304000  # 4GB
        LOGGER.info("Setting LEECH_SPLIT_SIZE to 4GB for Premium user")
    else:
        # For non-premium users, use standard 2GB
        Config.LEECH_SPLIT_SIZE = 2097152000  # 2GB
        LOGGER.info("Setting LEECH_SPLIT_SIZE to 2GB for standard user")

    # Always enable these features for premium users
    if TgClient.IS_PREMIUM_USER:
        Config.HYBRID_LEECH = True
        Config.USER_TRANSMISSION = True
        LOGGER.info("Enabled HYBRID_LEECH and USER_TRANSMISSION for Premium user")
    else:
